...
namespace Panzers.AI
{
    ...
    public class ThereIsMyVulnerableUnitInEnemyRange : ConditionActionBase
    {
            ...
            IUtilityMatchQueries queryMatchUtility)
        {
            ...
            if (FindMyVulnerableUnit(generalMatchUtility, myUnits, enemyUnits, ref myVulnerableUnit,
                ref selectedEnemyUnit))
                return null;

            if (FindRightPlaceToEscape(generalMatchUtility, queryMatchUtility, myVulnerableUnit, selectedEnemyUnit,
                out var execute)) return execute;
        ...
        {
            for (int i = myVulnerableUnit.FieldCoordinate.Data.X - 3;
                i < myVulnerableUnit.FieldCoordinate.Data.X + 3;
                i++)
            {
                for (int j = myVulnerableUnit.FieldCoordinate.Data.Y - 3;
                    j < myVulnerableUnit.FieldCoordinate.Data.X + 3;
                    j++)
                {
        ...
        private static bool FindMyVulnerableUnit(IUtilityMatchGeneral generalMatchUtility,
            List<FightingUnitMonoBase> myUnits, List<FightingUnitMonoBase> enemyUnits,
